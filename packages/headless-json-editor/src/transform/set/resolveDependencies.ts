...
export function resolveDependencies(
    ...
    if (currentlyEditedChild === undefined) {
        throw new Error(
            `currently edited child '${node.pointer}/${pointerToValue[0]}' could not be resolved on transform.set`
        );
    }
    // rebuild this object with updated data
    const currentData = getUpdatedData<UnknownObject>(node, join(pointerToValue), value);
    // @change delete node
    changeSet.push({ type: 'delete', node: node });
    // note: we may not replace edited child, but must update it
    const newNode = create<ObjectNode>(draft, currentData, node.schema, node.pointer);
    // @change create node
