...
To ensure oneOf schemas can be distinguished it is recommended to add a unique property:
...
> _Note_ You can set `{ options: { hidden: true }}` to each property _id_ to hide the control variable from user interface. Still, the id will be exported in data.
