...
- ❎ [**additionalProperties**](#additional-properties) - missing a ui to add any data
...
- ✅ [**if-then-else**](#if-then-else) - Attention: subschema is merged
...
Per default or with `addOptionalProps` set to `false` only required properties will be added when missing. This behaviour corresponds to json-schema `additionalProperties` being `true` per default. Setting `addOptionalProps=true` will add all missing properties per default:
...
<Canvas>
  <JsonForm
    ...
    schema={{
      ...
      required: ['one'],
      properties: {
        one: { title: 'Property One is required', type: 'string' },
        two: { title: 'Property Two is optional', type: 'string' }
      },
      options: { title: false }
    }}
  />
