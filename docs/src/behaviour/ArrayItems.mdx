...
<Meta title="Schema Behaviour/ArrayItems" />
...
Using `prefixItems` schema you define a JSON Schema for specific array items. `maxItems` and `minItems` now restrict the number of items that can be added. Setting `maxItems: 2` will disable add-options:

<Tabs>
  <Canvas tabTitle="draft-2020-12" className="inline" of={ArrayItems.ItemsArray} />
</Tabs>

Until **Draft-2019-09** Using `items[]` schema you define a JSON Schema for a fixed numer of array items. With no additional items defined,
...
An `items` object-schema defines the structure for all array-items which are not defined by an array-schema (items,
